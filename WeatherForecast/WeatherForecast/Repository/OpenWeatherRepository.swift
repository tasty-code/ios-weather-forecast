...
final class OpenWeatherRepository {
    ...
    func fetchForecast(lattitude: Double, longitude: Double, completion: @escaping (Result<Forecast, NetworkError>) -> Void) {
        ...
        let task = URLSession.shared.dataTask(with: url) { data, response, error in
            ...
            do {
                let currentForecast = try decoder.decode(Forecast.self, from: data)
                completion(.success(currentForecast))
            } catch {
                completion(.failure(.parseError))
            }
        }
